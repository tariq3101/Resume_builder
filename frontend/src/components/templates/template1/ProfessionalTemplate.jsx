      ...
      {personalInfo.links && personalInfo.links.length > 0 && (
        <div className="links-section">
          {personalInfo.links.map((link, idx) => {
            const hostname = new URL(link).hostname.replace('www.', ''); // Extract hostname without 'www.'
            return (
              <p key={idx}>
        ...
        {workExperience.map((job, idx) => (
          <div key={idx} className="professional-work">
            <h4 className="job-title">
              {job.role || "Role"} - <span className="company">{job.company || "Company"}</span>
            </h4>
            <p className="work-description">{job.description || "Description of work done"}</p>
      ...
      <section className="professional-section">
        ...
        {awards.map((award, idx) => (
          <div key={idx} className="award-item">
            <strong>{award.name || "Award Name"}</strong> - {award.issuer || "Issuer"}
      ...
      <section className="professional-section">
        ...
        {projects.map((project, idx) => (
          <div key={idx} className="project-item">
            ...
            <p><strong>Description:</strong> {project.description || "Project Description"}</p>
            {project.techStack && <p><strong>Technologies:</strong> {project.techStack.join(", ")}</p>}
            {project.link && <p><strong>Project Link:</strong> <a href={project.link} target="_blank" rel="noopener noreferrer">View Project</a></p>}
          </div>
        ))}
      </section>
    )}
